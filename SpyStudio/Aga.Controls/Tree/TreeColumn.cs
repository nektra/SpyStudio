...
namespace Aga.Controls.Tree
{
	...
	public class TreeColumn : Component
	{
        ...
        public Size GetMinimumSizeToFitHeader(Graphics gr, Font font)
        {
            Size maxTextSize = TextRenderer.MeasureText(gr, Header, font, new Size(0,0), TextFormatFlags.NoPadding);
            maxTextSize.Width += HeaderLeftMargin + HeaderRightMargin;

            if (SortOrder != SortOrder.None)
                maxTextSize.Width -= (SortMarkSize.Width + SortOrderMarkMargin);
		...
		internal static void DrawBackground(Graphics gr, Rectangle bounds, bool pressed, bool hot)
		{
			...
			{
				CreateRenderers();
                // HACK: to make the header look exactly like MS TreeView I've modified the bounds to DrawBackground
			    Rectangle bounds2 = new Rectangle(bounds.X, bounds.Y+1, bounds.Width, bounds.Height-1);
				if (pressed)
					_pressedRenderer.DrawBackground(gr, bounds2);
				else if (hot)
					_hotRenderer.DrawBackground(gr, bounds2);
				else
					_normalRenderer.DrawBackground(gr, bounds2);
			}
			else
			{
