...
namespace Aga.Controls.Tree
{
	public class AutoRowHeightLayout: IRowLayout
	{
		...
		public int PageRowCount
		{
			get 
			{
				if (_treeView.ReachableNodeCount == 0)
					return 0;
				else
				{
					int pageHeight = _treeView.DisplayRectangle.Height - _treeView.ColumnHeaderHeight;
					int y = 0;
					for (int i = _treeView.ReachableNodeCount - 1; i >= 0; i--)
					{
						y += GetRowHeight(i);
						if (y > pageHeight)
							return Math.Max(0, _treeView.ReachableNodeCount - 1 - i);
					}
					return _treeView.ReachableNodeCount;
				}
			}
		}
		...
		public int CurrentPageSize
		{
			get
			{
				...
				else
				{
					int pageHeight = _treeView.DisplayRectangle.Height - _treeView.ColumnHeaderHeight;
					int y = 0;
					for (int i = _treeView.FirstVisibleRow; i < _treeView.ReachableNodeCount; i++)
					{
						y += GetRowHeight(i);
						if (y > pageHeight)
							return Math.Max(0, i - _treeView.FirstVisibleRow);
					}
					return Math.Max(0, _treeView.ReachableNodeCount - _treeView.FirstVisibleRow);
				}
			}
		}
		...
		{
			if (rowNo < _treeView.ReachableNodeCount)
			{
				TreeNodeAdv treeNode = _treeView.GetTreeNodeByRow(rowNo);
				...
				if (treeNode.Height == null)
				{
					...
					{
						int h = nc.GetActualSize(treeNode, _measureContext).Height;
						if (h > res)
							res = h;
					}
					treeNode.Height = res;
				}
				return treeNode.Height.Value;
			}
			else
				return 0;
		}
