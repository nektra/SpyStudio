...
namespace Aga.Controls.Tree.NodeControls
{
	public abstract class BaseTextControl : EditableControl
	{
        ...
        public bool CompactString { get; set; }
		...
		protected Font GetDrawingFont(TreeNodeAdv node, DrawContext context, string label)
		{
			...
			{
				DrawTreeEventArgs args = new DrawTreeEventArgs(node, this, context, label);
		...
		{
			if (context.CurrentEditorOwner == this && node.Node == Parent.CurrentNode)
				return;
            ...
            if (UseCompatibleTextRendering)
                context.Graphics.DrawString(label, font,
                                            GetFrush(textColor),
                                            bounds, _format);
            else
                TextRenderer.DrawText(context.Graphics, label, font, bounds, textColor,
                                      CompactString ? (_formatFlags | TextFormatFlags.PathEllipsis) : _formatFlags);
		...
		private void CreateBrushes(TreeNodeAdv node, DrawContext context, string text, out Brush backgroundBrush, out Color textColor, out Font font, ref string label)
		{
			...
			if (DrawTextMustBeFired(node))
			{
				DrawTreeEventArgs args = new DrawTreeEventArgs(node, this, context, text);
				args.Text = label;
		...
		/// </summary>
		public event EventHandler<DrawTreeEventArgs> DrawText;
		protected virtual void OnDrawText(DrawTreeEventArgs args)
		{
