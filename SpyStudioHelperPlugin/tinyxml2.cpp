...
XMLError XMLDocument::Parse( const wchar_t* p, size_t len )
{
    ...
    _charBuffer = new char[ len+1 ];
    const char* start = _charBuffer;
    for (size_t i = 0; i < len; i++)
        _charBuffer[i] = (char)p[i];
    _charBuffer[len] = 0;

    start = XMLUtil::SkipWhiteSpace( start );
    start = XMLUtil::ReadBOM( start, &_writeBOM );
    if ( !p || !*p ) {
        SetError( XML_ERROR_EMPTY_DOCUMENT, 0, 0 );
        return _errorID;
    }

    ptrdiff_t delta = start - _charBuffer;	// skip initial whitespace, BOM, etc.
    ParseDeep( _charBuffer+delta, 0 );
    return _errorID;
}
